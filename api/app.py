...
# Cache for image URLs (1-hour TTL)
    ...
    return [
        {
            ...
            "space_agency": "NASA",
            "notable_missions": ["Apollo 11"],
            "current_status": "deceased",
            "image_url": "https://example.com/neil_armstrong.jpg"
        }
    ] * 20
...
# Reusable Groq API call with retries
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10))
async def call_groq_api(prompt: str, model: str = 'llama3-8b-8192', max_tokens: int = 4096, temperature: float = 0.3, timeout: int = 30) -> str:
...
async def scrape_astronauts(limit: int = 20) -> List[Dict[str, any]]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        return astronauts if astronauts else mock_astronauts()
...
async def scrape_missions(limit: int = 30) -> List[Dict[str, any]]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        return missions if missions else mock_missions()

@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10))
async def scrape_quiz_questions(limit: int = 10) -> List[Dict[str, any]]:
    url = "https://www.space.com/space-quizzes"
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    async with aiohttp.ClientSession() as session:
        try:
            html = await fetch_url(session, url, headers)
            ...
            return questions if questions else mock_quiz_questions()
        except Exception:
...
async def fetch_pixabay_image(query: str) -> str:
    ...
    pixabay_url = f"https://pixabay.com/api/?key={PIXABAY_API_KEY}&q={query}&image_type=photo&category=science&safesearch=true"
    async with aiohttp.ClientSession() as session:
        try:
...
async def get_astronauts():
    ...
    try:
        content = await call_groq_api(prompt)
...
async def get_astronaut_details(data: AstronautName):
    ...
    try:
        content = await call_groq_api(prompt, max_tokens=2048)
        content = content.strip()
        if content.startswith('{') and not content.endswith('}'):
            content += '}'
...
async def get_nasa_stats():
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(NEO_FEED_URL, params=neo_params) as response:
                response.raise_for_status()
        ...
        - missions_by_type: JSON object with mission types as keys and number of missions as values.
        Format as a valid JSON object.'''
        
        content = await call_groq_api(prompt, max_tokens=512)
        content = content.strip()
        if content.startswith('{') and not content.endswith('}'):
            content += '}'
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching NASA stats: {str(e)}")
...
async def get_articles(date: Optional[str] = None, query: Optional[str] = None):
    ...
    try:
        content = await call_groq_api(prompt, max_tokens=1024, temperature=0.7)
        content = content.strip()
        if content.startswith('[') and not content.endswith(']'):
            content += ']'
...
def mock_articles():
    return [
        {"title": "NASA's James Webb Telescope Discovers New Exoplanet", "summary": "The James Webb Space Telescope has identified a new Earth-like exoplanet.", "date": "2025-03-09", "link": "https://example.com", "imageUrl": "https://example.com/jwst.jpg"},
        {"title": "SpaceX Tests Starship", "summary": "SpaceX conducted a successful test of its Starship spacecraft.", "date": "2025-03-08", "link": "https://example.com", "imageUrl": "https://example.com/starship.jpg"}
    ]
...
async def chat(data: ChatMessage):
    if not GROQ_API_KEY:
        return {"response": "Chat unavailable without Groq API key."}
    
    chat_context = [
        {"role": "system", "content": "You are a helpful space and astronomy assistant."},
        {"role": "user", "content": data.message}
    ]
    
    try:
        content = await call_groq_api(json.dumps(chat_context), max_tokens=800, temperature=0.7)
        return {"response": content}
    except Exception as e:
        return {"error": str(e), "response": "Error processing chat request."}
