...
export function Home() {
    ...
    const fetchApodData = async () => {
      try {
        ...
        if (cachedApod) {
          ...
          const apodRes = await axios.get(`${process.env.BACKEND_API}/api/nasa/apod`);
  ...
  const fetchWeatherData = async (latitude: number, longitude: number) => {
    try {
      ...
      const weatherRes = await axios.get(`${process.env.BACKEND_API}/api/space-weather`, {
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      try {
        const response = await axios.get(`${process.env.BACKEND_API}/api/articles?date=${today}`);
  ...
  const handleSearch = async (query: string) => {
    try {
      setLoading(true);
      const response = await axios.get(`${process.env.BACKEND_API}/api/articles?query=${query}`);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.BACKEND_API}/api/chat`, { message: userInput });
