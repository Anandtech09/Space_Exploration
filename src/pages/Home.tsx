...
export function Home() {
    ...
    const fetchApodData = async () => {
      try {
        ...
        if (cachedApod) {
          ...
          const apodRes = await axios.get('http://localhost:5000/api/nasa/apod');
  ...
  const fetchWeatherData = async (latitude: number, longitude: number) => {
    try {
      ...
      const weatherRes = await axios.get('http://localhost:5000/api/space-weather', {
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/articles?date=${today}`);
  ...
  const handleSearch = async (query: string) => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/articles?query=${query}`);
